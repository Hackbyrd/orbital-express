 ...
 * TODO: This is a todo
 */
async function V1Read2(req) {
  const schema = joi.object({
    alpha: joi.string().trim().min(1).lowercase().required().error(new Error(req.__('ORDER_V1Read2_Invalid_Argument[alpha]'))),
    beta: joi.boolean().default(true).optional().error(new Error(req.__('ORDER_V1Read2_Invalid_Argument[beta]'))),
    gamma: joi.number().integer().min(1).max(10).error(new Error(req.__('ORDER_V1Read2_Invalid_Argument[gamma]'))),
    delta: joi.string().trim().lowercase().min(3).email().required().error(new Error(req.__('ORDER_V1Read2_Invalid_Argument[delta]'))),
    zeta: joi.string().trim().valid('a', 'b').required().error(new Error(req.__('ORDER_V1Read2_Invalid_Argument[zeta]')))
  ...
  try {
    ...
    const job = await OrderQueue.add('V1Read2Task', data);
