 ...
 * node scripts/generate generate|gen|-g [NEW_FEATURE_FOLDER_NAME]
 * node scripts/generate generate|gen|-g [EXISTING_FEATURE_FOLDER_NAME] action|-a [NEW_ACTION_FILE_NAME]
 * node scripts/generate generate|gen|-g [EXISTING_FEATURE_FOLDER_NAME] task|-t [NEW_TASK_FILE_NAME]
 * node scripts/generate generate|gen|-g [EXISTING_FEATURE_FOLDER_NAME] mail|-m [NEW_MAILER_FILE_NAME]
 * node scripts/generate delete|del|-d [NEW_FEATURE_FOLDER_NAME]
 * node scripts/generate delete|del|-d [EXISTING_FEATURE_FOLDER_NAME] action|-a [NEW_ACTION_FILE_NAME]
 * node scripts/generate delete|del|-d [EXISTING_FEATURE_FOLDER_NAME] task|-t [NEW_TASK_FILE_NAME]
 * node scripts/generate delete|del|-d [EXISTING_FEATURE_FOLDER_NAME] mail|-m [NEW_MAILER_FILE_NAME]
 * node scripts/generate stringify|str|-s [ABSOLUTE_PATH_OF_FILE_TO_STRINGIFY]
 *
 *
...
// app/ directory where feature folders live
...
function generate() {
  ...
  const newDirPath = path.join(APP_DIR, newDir);
  ...
  /***** Update Sequence.js *****/
  /******************************/
  const sequenceArray = require(path.join(ROOT_DIR, 'database/sequence'));

  // add new feature name
  sequenceArray.push(camelName);
  const featureNames = sequenceArray.map(name => `'${name}'`).join(',\n  ');

  fd = fs.openSync(path.join(ROOT_DIR, 'database/sequence.js'), 'w');
...
function generateAction() {
  ...
  const featDirPath = path.join(APP_DIR, featDir);
  ...
  // remove [''] if the file does not have any require files (empty)
...
function destroy() {
  ...
  const rmDirPath = path.join(APP_DIR, rmDir);
  ...
  // file descriptor
  let fd = fs.openSync(path.join(ROOT_DIR, 'database/sequence.js'), 'w');
  fs.writeSync(fd, sequenceFileText(featureNames), 0, 'utf-8');
  fs.closeSync(fd);
...
function destroyAction() {
  ...
  requireActionFiles = requireActionFiles.filter(actionFile => actionFile.indexOf(requireText) < 0); // will filter out the targeted file
  requireActionFiles = requireActionFiles.sort(); // sort alphabetically
  let requireActionFilesStr = requireActionFiles.join(',\n  ');
...
function destroyTask() {
  ...
  // update task index file
  let rmTaskIndexFileText = fs.readFileSync(rmTasksIndexPath, 'utf8').replace(/ /g, '').replace(/\n/g, '').replace(/\t/g, ''); // grab file text and remove all spaces
  let requireTaskFiles = rmTaskIndexFileText.substring(rmTaskIndexFileText.indexOf('module.exports')).split('{')[1].split('}')[0].split(','); // get all the tasks
  let requireText = `...require('./${rmFileName}')`; // the require code
  requireTaskFiles = requireTaskFiles.filter(actionFile => actionFile.indexOf(requireText) < 0); // will filter out the targeted file
  requireTaskFiles = requireTaskFiles.sort(); // sort alphabetically
  let requireTaskFilesStr = requireTaskFiles.join(',\n  ');
